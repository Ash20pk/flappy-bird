...
const Dashboard = () => {
  const { playerStats, loading } = useContext(PlayerContext);
      ...
      <div className="w-full max-w-md bg-yellow-300 rounded-xl shadow-lg p-6 relative z-10">
        <div className="relative">
          ...
          <Link to="/game" className="block">
            <button className="w-full bg-red-500 hover:bg-red-600 text-white font-bold py-6 px-4 rounded-full transition duration-300 ease-in-out transform hover:scale-105 flappy-font text-3xl shadow-lg">
              PLAY NOW
            </button>
          </Link>
        </div>
